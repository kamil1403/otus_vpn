...
2. Генерация PKI (CA, сертификаты, ключи) через Easy-RSA.
3. Настройка OpenVPN сервера и клиента.
4. Проверка пропускной способности канала.

### ✅ Результат
- [x] Автоматический Provisioning всей сети.
- [x] Настроен роутинг и форвардинг трафика.
- [x] Успешные тесты iperf3 (270+ Mbits/sec).
- [x] Конфиги клиента генерируются автоматически в `client_keys/`.
...
* **PKI**: Инициализация Easy-RSA, генерация CA, сертификатов сервера и клиента.
* **Server**: Настройка `server.conf`, включение IP Forwarding, настройка IPTables (MASQUERADE).
